...
// The project layout guards URL ↔ store sync — if we reached this page,
// currentProject is the right one.

// ============================================================
// Form state — pre-filled from current project values
// ============================================================
...
<template>
  <main v-if="currentProject" class="mx-auto max-w-3xl px-6 py-16">
    <div class="space-y-2 mb-10">
      <p class="text-xs uppercase tracking-widest text-accent-blue-bright">{{ t('editProject.kicker') }}</p>
      <h1 class="text-3xl font-semibold text-ink-high">{{ t('editProject.title') }}</h1>
      <p class="text-ink-mid">{{ t('editProject.subtitle') }}</p>
    </div>
    ...
    <form class="space-y-10" @submit.prevent="submit">
      ...
      <div class="space-y-3">
        <div class="space-y-1">
          <label class="text-xs uppercase tracking-wider text-ink-mid">{{ t('module0.sectorLabel') }}</label>
